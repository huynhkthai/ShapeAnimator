...
public class AnimationReader {
   ...
   * Model.Shape lines: the keyword "shape" followed by two identifiers (i.e.
   * alphabetic strings with no spaces), giving the unique name of the shape,
   * and the type of shape it is.
   *Motion lines: the keyword "motion" followed by an identifier giving the name
   ...
   * @return A complete Builder that has read through all the data and called all the methods.
   */
  public static AnimationBuilder parseFile(Readable readable,
                                           AnimationBuilder builder) {
    Objects.requireNonNull(readable, "Must have non-null readable source");
    Objects.requireNonNull(builder, "Must provide a non-null Model.AnimationBuilder");
  ...
  /**
   * This method reads the canvas information from the txt file and call on builder to convert it to
   * usable data.
   *
   * @param s       looks for input in canvas format.
   * @param builder is the mechanism that converts raw data to usable data.
   */
  ...
  private static void readShape(Scanner s, AnimationBuilder builder) {
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException("Model.Shape: Expected a name, but no more input available");
    }
    if (s.hasNext()) {
      type = s.next();
    } else {
      throw new IllegalStateException("Model.Shape: Expected a type, but no more input available");
    }
